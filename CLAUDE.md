...
| PATCH | `/api/v1/admin/data-quality/roster-quality-flags/:id/resolve` | Operator triage: stamps `resolved_at = NOW()` and `resolved_by = <admin user id>` on a `roster_quality_flags` row (NULL for API-key callers — same pattern as the dedup PATCH endpoints). Optional body `{notes?: string}` (1k cap). Idempotent on re-resolve (204); 404 only when the flag id is unknown. |
