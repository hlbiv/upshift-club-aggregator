...
| `club_events` | **Deferred drop** — superseded by `events` + `event_teams` |
| `club_coaches` | **Deferred drop** — backfilled into `coach_discoveries` |
...
- ⏳ **Next:** PR to drop `club_coaches` and `club_events` (both deferred — schema entries remain until the drop PR lands)
