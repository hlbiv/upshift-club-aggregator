...
def _run_rollup(args) -> None:
    ...
    if key == "retention-prune":
        ...
        if run_log is not None:
            # records_touched == created + updated + failed; deletions are
            # neither creates nor updates, so we record them under
            # records_updated to keep the row visible in dashboards. No
            # failures expected; pruning is set-based and idempotent.
            run_log.finish_ok(
                records_created=0,
                records_updated=(
                    result["scrape_run_logs_deleted"]
...
def main() -> None:
    ...
    parser.add_argument("--rollup", choices=["club-results", "retention-prune"],
