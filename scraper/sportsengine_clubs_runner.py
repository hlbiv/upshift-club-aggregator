"""
sportsengine_clubs_runner.py — Orchestrate the SportsEngine + JSON-LD
club extractor across the canonical_clubs seed list.

Invoked via ``run.py --source sportsengine-clubs``.

Per club with a non-empty ``website``:
  1. HEAD/GET the homepage; bail unless ``cms_detect.detect_cms``
     returns ``'sportsengine'``.
  2. Aggregate JSON-LD across the homepage + a fixed list of common
     subpaths PLUS SE-conventional ``/page/show/<id>-<slug>`` URLs
     discovered from the homepage's anchor hrefs.
  3. Route typed nodes into the four sinks:
       - ``SportsTeam.athlete[]`` → ``club_roster_snapshots``
       - ``Person`` blocks         → ``coach_discoveries`` (tagged
...
Fails soft: one bad URL never aborts the whole run; the per-site
failure is dropped and the runner continues. The ``--limit`` flag caps
how many candidate clubs to probe per invocation (default 50) —
operational tuning happens after the first manual run shows real
coverage numbers.

This runner is the SE sibling of ``squarespace_clubs_runner`` (PR #58).
Where the two diverge:

  * Coach rows are stamped ``platform_family='sportsengine'`` (the
    enum already includes this value, so no schema change required).
  * The harvest tracks a count of auth-walled endpoints
    (``HTTP 401/403``) per site, surfaced in the run summary so the
    operator can see how often the SE widget API blocks us.
  * Otherwise the writers are identical — same idempotent upserts
    against the same five tables.
"""
...
logger = logging.getLogger("sportsengine_clubs_runner")
...
@dataclass
class SportsEngineRunOutcome:
    sites_considered: int = 0
    sites_sportsengine: int = 0
    pages_fetched: int = 0
...
def _get_connection():
    ...
    Mirrors the soft-fail pattern in ``youth_club_coaches`` and
    ``squarespace_clubs_runner`` — the runner can be invoked in dry-run
    from a laptop without DB access and still report progress (per
    CLAUDE.md "scraping continues regardless").
    """
    if psycopg2 is None:
        return None
    ...
    try:
        ...
        logger.warning("[sportsengine-clubs] DB connect failed: %s", exc)
        return None
...
def _fetch_candidates(
    ...
    *,
    ...
    sites: List[SportsEngineClubSite] = []
...
def _write_harvest(
    ...
    *,
    ...
    if conn is None:
        ...
        logger.warning(
            "[sportsengine-clubs] no DB connection; would have written "
    ...
    if harvest.roster_rows:
        ...
        try:
            ...
            logger.warning(
                "[sportsengine-clubs] roster write failed for %s: %s",
    ...
    # the other writers, and so updates of `platform_family` flow
    # through correctly when a SE coach previously landed via the
    # Squarespace ('unknown') extractor.
    if harvest.coach_rows:
        try:
                ...
                for row in harvest.coach_rows:
                    cur.execute(
                        ...
                        (
                            ...
                            row.get("source_url"),
                            row.get("confidence"),
                            row.get("platform_family") or "sportsengine",
                        ),
                    )
                    res = cur.fetchone()
                    if res and res[0]:
            ...
            logger.warning(
                "[sportsengine-clubs] coach write failed for %s: %s",
                harvest.club_name, exc,
            )
            try:
                conn.rollback()
    ...
    if harvest.tryout_rows:
        ...
        try:
            ...
            logger.warning(
                "[sportsengine-clubs] tryouts write failed for %s: %s",
                harvest.club_name, exc,
            )
            try:
                conn.rollback()
    ...
    if harvest.enrichment_row is not None:
        ...
        try:
            ...
            logger.warning(
                "[sportsengine-clubs] enrichment write failed for %s: %s",
                harvest.club_name, exc,
            )
            try:
                conn.rollback()
...
def run_sportsengine_clubs(
    *,
    dry_run: bool = False,
    limit: Optional[int] = DEFAULT_LIMIT,
    state: Optional[str] = None,
) -> SportsEngineRunOutcome:
    """Probe canonical_clubs websites for SportsEngine + JSON-LD.
    ...
    """
    outcome = SportsEngineRunOutcome()
    counts: Dict[str, int] = {
    ...
    if not dry_run:
        run_log = ScrapeRunLogger(
            ...
            league_name="SportsEngine JSON-LD clubs",
        )
    ...
    if conn is None and not dry_run:
        logger.error(
            "[sportsengine-clubs] DATABASE_URL not set; aborting (dry-run "
            "OK without DB)"
        )
    ...
    try:
        if conn is None:
            sites: List[SportsEngineClubSite] = []
            logger.warning(
                "[sportsengine-clubs] no DB connection — dry-run with empty "
            ...
            try:
                ...
                logger.error(
                    "[sportsengine-clubs] candidate fetch failed: %s", exc,
                )
                if run_log is not None:
        ...
        logger.info(
            "[sportsengine-clubs] probing %d candidate club(s) (limit=%s, state=%s)",
        ...
        try:
            for site in sites:
                try:
                    ...
                    logger.warning(
                        "[sportsengine-clubs] crashed on %s: %s",
                        site.club_name_canonical, exc,
                    )
                    continue

                outcome.pages_fetched += harvest.pages_fetched
                outcome.auth_walled_endpoints += harvest.auth_walled_endpoints
                if not harvest.is_sportsengine:
                    continue
                outcome.sites_sportsengine += 1
        ...
        logger.info(
            "[sportsengine-clubs] done: candidates=%d sportsengine=%d "
            "pages=%d auth-walled=%d inserted=%d updated=%d",
            outcome.sites_considered, outcome.sites_sportsengine,
            outcome.pages_fetched, outcome.auth_walled_endpoints,
...
def print_summary(outcome: SportsEngineRunOutcome) -> None:
    print("\n" + "=" * 60)
    print("  SportsEngine JSON-LD clubs — run summary")
    print("=" * 60)
    print(f"  Candidates considered : {outcome.sites_considered}")
    print(f"  Detected SportsEngine : {outcome.sites_sportsengine}")
    print(f"  Pages fetched         : {outcome.pages_fetched}")
    print(f"  Auth-walled endpoints : {outcome.auth_walled_endpoints}")
    c = outcome.counts
